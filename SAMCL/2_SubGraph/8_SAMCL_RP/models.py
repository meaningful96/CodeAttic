...
class CustomBertModel(nn.Module, ABC):
    def __init__(self, args):
        ...
        for item in self.train_data:
            ...
            if item['relation'] not in self.relation_type_dict:
                self.relation_type_dict[item['relation']] = cnt
                cnt += 1
        ...
        num_relations = len(list(self.relation_type_dict))
        self.linear = nn.Linear(self.config.hidden_size, num_relations)
    ...
    def forward(self, hr_token_ids, hr_mask, hr_token_type_ids,
                ...
                ht_token_ids, ht_mask, ht_token_type_ids,
        ...
        ht_vector = self._encode(self.hr_bert,
                                 token_ids=ht_token_ids,
                                 mask=ht_mask,
                                 token_type_ids=ht_token_type_ids)
    ...
    def compute_logits(self, output_dict: dict, batch_dict: dict) -> dict:
        ...
        ht_vector = output_dict['ht_vector']
        ...
        labels_rp = torch.zeros(batch_size, dtype=torch.long).to(hr_vector.device)
        for i, ex in enumerate(batch_data_forward):
            labels_rp[2*i] = self.relation_type_dict[ex[1]]
            labels_rp[2*i+1] = self.relation_type_dict[ex[1]]
        ...
        ## Loss for RP
        logits_rp = self.linear(ht_vector).to(hr_vector.device)
        logits_rp = F.softmax(logits_rp, dim=1).float()
        logits_rp = logits_rp.float()
