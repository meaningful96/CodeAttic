...
def random_walk_for_example(example, num_negative, G):
    ...
    positive_samples, sampling_count = G.random_walk(head_id, relation, tail_id, args.randomwalk_step, num_negative, args.iteration)
    return positive_samples

def process_data(data, num_negative, G):
    all_positive_samples = []
    for example in data:
        positive_samples = random_walk_for_example(example, num_negative, G)
        all_positive_samples.extend(positive_samples)
    return all_positive_samples

def randomwalk_sampling(data, graph, num_negative, num_process=45):
    pool = multiprocessing.Pool(processes = num_process)
    chunks = [data[i:i + len(data) // num_process] for i in range(0, len(data), len(data) // num_process)]
    results = pool.starmap(process_data, [(chunk, graph, num_negative) for chunk in chunks])
    all_positive_samples = []
