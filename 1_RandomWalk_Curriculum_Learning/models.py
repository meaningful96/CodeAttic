...
from triplet_mask import construct_mask_hr, construct_mask_tail, construct_mask, construct_self_negative_mask, construct_SN_mask
...
class CustomBertModel(nn.Module, ABC):
    def __init__(self, args):
        ...
        self.random_vector = torch.randn(1,1)
    ...
    def compute_logits(self, output_dict: dict, batch_dict: dict) -> dict:
        ...
        if args.validation:
            ...
            triplet_mask = construct_mask(row_exs = batch_dict['batch_data']).to(logits.device)
            if triplet_mask is not None:
                logits.masked_fill_(~triplet_mask, -1e4)
                swapped_tail = tail_vector.clone()
            swapped_tail[::2], swapped_tail[1::2] = swapped_tail[1::2].clone(), swapped_tail[::2].clone()
            self_neg_logits = torch.sum(hr_vector * swapped_tail, dim=1) * self.log_inv_t_hard.exp()
            self_negative_mask = construct_self_negative_mask(batch_dict['batch_data']).to(logits.device)
            self_neg_logits.masked_fill_(~self_negative_mask, -1e4)
            logits = torch.cat([logits, self_neg_logits.unsqueeze(1)], dim=-1)
            
            ### Never use in validation step ###
            logits_hr = self.random_vector
            logits_tail = self.random_vector
            logits_reverse = self.random_vector           
            labels_hr = self.random_vector 
            labels_tail = self.random_vector
            ####################################
           
        if args.cur and not args.validation:
            """
            ## same with validation
            ## At Epoch = 0
            ## The logits are same as original SimKGC
            """
        ...
        if not args.cur and not validation:
            """
            ...
            labels_hr = torch.arange(0,int(self.batch_size // 2), int(self.negative_size//2)).long().to(hr_vector.device)
            labels_tail = torch.arange(self.batch_size // self.negative_size).to(hr_vector.device)
            ...
            if self.args.use_self_negative and self.training:
                self_neg_logits = torch.sum(hr1 * hr_reverse, dim=1) * self.log_inv_t_hard.exp()
                self_negative_mask = construct_SN_mask(batch_dict['batch_data']).to(logits_hr.device)
                self_neg_logits.masked_fill_(~self_negative_mask, -1e4)
        ...
        return {'logits': logits,
                ...
                'inv_t_hard': self.log_inv_t_hard.detach().exp(),
