...
class Dataset(torch.utils.data.dataset.Dataset):
    ...
    def __init__(self, data, task, batch_indices=None, step_size = None, negative=False):
               
        if negative == False: 
            if batch_indices is None:
                batch_indices = random.sample(range(len(data)), step_size)
...
def load_data(path: str, graph, directed_graph, walks_num, add_forward_triplet: bool = True,
    ...
    all_pathes, candidates = D.intermediate()
    ...
    for path in range(len(all_pathes)):
        for triplet in all_pathes[path]:
    ...
    logger.info('never appear in original dfs pathes: {}'.format(not_seen_path))
    ...
    logger.info(f'time to do dfs: {datetime.timedelta(seconds = (end-start))}')
    ...
    return all_pathes, all_examples, candidates
