...
class Trainer:
    ...
    def train_loop(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            sampling_train = randomwalk_sampling(self.train_dataset, self.G_train)
            sampling_valid = randomwalk_sampling(self.valid_dataset, self.G_valid)

            train_data = shuffle_dataset(sampling_train, self.negative_size * 2)
            valid_data = shuffle_dataset(sampling_valid, self.negative_size * 2)            
    ...
    def _run_eval(self, epoch, step=0):
        ...
        is_best = self.valid_loader and (self.best_metric is None or metric_dict['Acc@3'] > self.best_metric['Acc@3'])
        if is_best:
    ...
    def eval_epoch(self, epoch) -> Dict:
        ...
        for i, batch_dict in enumerate(self.valid_loader):
            ...
            logits_reverse, labels_reverse = outputs.logits_reverse, outputs.labels_tail
            loss = self.criterion(logits_hr, labels_hr)
            loss += self.criterion(logits_reverse, labels_reverse)
