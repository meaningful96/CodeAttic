...
class Example:
    ...
    def vectorize(self) -> dict:
        ...
        ht_encoded_inputs = _custom_tokenize(text=head_text,
                                             text_pair=tail_text)
        ...
        return {'hr_token_ids': hr_encoded_inputs['input_ids'],
                ...
                'ht_token_ids': ht_encoded_inputs['input_ids'],
                'ht_token_type_ids': ht_encoded_inputs['token_type_ids'],
                'triple': triple,
                'obj': self}
...
def collate(batch_data: List[dict]) -> dict:
    ...
    ht_token_ids, ht_mask = to_indices_and_mask(
        [torch.LongTensor(ex['ht_token_ids']) for ex in batch_data],
        pad_token_id=get_tokenizer().pad_token_id)
    ht_token_type_ids = to_indices_and_mask(
        [torch.LongTensor(ex['ht_token_type_ids']) for ex in batch_data],
        need_mask=False)
    ...
    batch_dict = {
        ...
        'ht_token_ids': ht_token_ids,
        'ht_mask': ht_mask,
        'ht_token_type_ids': ht_token_type_ids,
