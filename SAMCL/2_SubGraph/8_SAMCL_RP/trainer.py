...
from randomwalk import Biased_RandomWalk, build_graph, Path_Dictionary, Making_Subgraph
...
class Trainer:
    ...
    def __init__(self, args, ngpus_per_node):
        ...
        train_obj, valid_obj = Biased_RandomWalk(args.train_path), Biased_RandomWalk(args.valid_path)
    ...
    def train_epoch(self, epoch):
        ...
        for i, batch_dict in enumerate(self.train_loader):
            ...
            outputs = ModelOutput(**outputs)
            logits, labels = outputs.logits, outputs.labels
            logits_rp, labels_rp = outputs.logits_rp, outputs.labels_rp
            ...
            loss1 = mean_tensor(loss_forward)
            
            # tail -> head + relation
            loss_backward = self.criterion(logits[:, :batch_size].t(), labels) * degree_head

            loss1 += mean_tensor(loss_backward)
            loss2 = self.criterion2(logits_rp, labels_rp)
