...
from triplet_mask import construct_mask_hr, construct_mask_tail, construct_mask_valid, construct_self_negative_mask
...
class CustomBertModel(nn.Module, ABC):
    ...
    def compute_logits(self, output_dict: dict, batch_dict: dict) -> dict:
        ...
        if args.validation:
            labels_hr = torch.arange(batch_size).to(hr_vector.device)
            labels_tail = torch.arange(1).to(hr_vector.device)
            ...
            triplet_mask_hr = construct_mask_valid(row_exs = batch_dict['batch_data']).to(logits_hr.device)
            if triplet_mask_hr is not None:
                logits_hr.masked_fill_(~triplet_mask_hr, -1e4)
            logits_reverse = torch.randn(1,1).to(hr_vector.device)
            logits_tail = torch.randn(1,1).to(hr_vector.device)
        ...
        if not args.validation:
            ...
            true_head = head_vector[::self.negative_size].to(logits_hr.device)
            self_neg_logits = (torch.sum(hr1 * true_head, dim=1) * self.log_inv_t.exp()).to(logits_hr.device)
