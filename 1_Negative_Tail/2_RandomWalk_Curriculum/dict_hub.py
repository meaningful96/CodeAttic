...
def _init_train_triplet_dict():
    ...
    if not train_triplet_dict:
        train_triplet_dict = TripletDict(path_list=[args.linkGraph_path])


def _init_all_triplet_dict():
    global all_triplet_dict
    if not all_triplet_dict:
        path_pattern = '{}/*.txt.json'.format(os.path.dirname(args.linkGraph_path))
...
def _init_link_graph():
    global link_graph
    if not link_graph:
        link_graph = LinkGraph(train_path=args.linkGraph_path)
